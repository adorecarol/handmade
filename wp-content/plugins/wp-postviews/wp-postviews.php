	...
	function get_least_viewed($mode = '', $limit = 10, $chars = 0, $display = true) {
		...
		if($most_viewed) {
			foreach ($most_viewed as $post) {
				...
				$temp = str_replace("%POST_THUMB%", get_the_post_thumbnail($post->ID, 'large',  array('class'=>'img-responsive')), $temp);
...
if(!function_exists('get_most_viewed')) {
	function get_most_viewed($mode = '', $limit = 6, $chars = 0, $display = true) {
		global $wpdb;
		$views_options = get_option('views_options');
		$where = '';
		$temp = '';
